// openai-client.js - OpenAI/Groq API統合

class OpenAIClient {
  constructor(apiKey, provider = 'openai', groqApiKey = null) {
    this.apiKey = apiKey;
  ...
  async transcribe(audioBlob, options = {}) {
    try {
      ...
      // API呼び出し（文字起こしは常にOpenAI Whisperを使用）
  ...
  async summarize(transcript, options = {}) {
    try {
      ...
      // プロバイダーに応じたベースURLとAPIキーを選択
      const baseURL = this.provider === 'groq' ? this.groqBaseURL : this.openaiBaseURL;
      const apiKey = this.provider === 'groq' ? this.groqApiKey : this.apiKey;
