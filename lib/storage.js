...
class Storage {
  ...
  static DEFAULT_SETTINGS = {
    ...
    summaryPrompt: '',
    audioRetentionHours: 24,  // 音声保持期間（時間）
    maxAudioCount: 20         // 最大音声保持件数
  };
  ...
  static async loadSettings() {
    try {
      ...
      const settings = result.settings || { ...Storage.DEFAULT_SETTINGS };
  ...
  static async saveTranscript(transcript) {
    try {
      ...
      // 最新20件のみ保持
      if (transcripts.length > Storage.MAX_TRANSCRIPTS) {
        transcripts.splice(Storage.MAX_TRANSCRIPTS);
      }
  ...
  static createTranscript(data) {
    return {
      ...
      audioStored: data.audioStored || false,  // 音声がIndexedDBに保存されているか
      status: data.status || 'processing'  // 処理状態: processing, transcribed, completed, failed
    };
  }
}
